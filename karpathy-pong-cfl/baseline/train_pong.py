...
Based on Andrej Karpathy's famous 130-line implementation
...
def save_model(episode, reward_sum):
    """Save model and training statistics"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Save model
    model_path = f'models/pong_baseline_{timestamp}_ep{episode}.p'
    ...
    stats_path = f'models/stats_{timestamp}_ep{episode}.p'
...
def plot_training_progress():
    ...
    plt.tight_layout()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(f'results/training_progress_{timestamp}.png', dpi=150, bbox_inches='tight')
    plt.close()
...
print("Starting Pong training with Policy Gradients...")
...
while True:
    ...
    if done: # an episode finished
        ...
        print(f'Episode {episode_number:4d}: reward: {reward_sum:6.1f}, running mean: {running_reward:6.1f}')
        
        # Save model periodically
        if episode_number % 100 == 0:
            save_model(episode_number, reward_sum)
            plot_training_progress()
        
        # Save final model
        if episode_number % 1000 == 0:
            final_path = save_model(episode_number, reward_sum)
    ...
    if reward != 0: # Pong has either +1 or -1 reward exactly when game ends.
        print(f'Episode {episode_number}: {"Won!" if reward == 1 else "Lost..."} (reward: {reward})')
