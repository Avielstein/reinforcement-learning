...
class WaterWorld:
    ...
    def step(self, action: Tuple[float, float]) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        for item, item_reward in bad_collisions:
            if self.config.TERMINATE_ON_BAD_ITEM:
                # Use stronger penalty and terminate episode
                reward += self.config.BAD_ITEM_PENALTY
        ...
        done = episode_terminated or self.episode_step >= self.config.MAX_EPISODE_STEPS
